...
# ==========================================
# LOAD NOISY DATA  ← only change here
# ==========================================
data = sio.loadmat('D:\Capstone\matlab_project\capstone_sensor_data\z_202603_Capstone_LineSen128_ExtSr_SensorData_5dots_diag_-10dB_.mat')
sensor_data = data['sensor_data']   # (128, 1024)
...
# NORMALIZE
# ==========================================
img_norm = np.abs(img_DMAS) / (np.max(np.abs(img_DMAS)) + 1e-12)

# ==========================================
# THRESHOLD ← lowered to 0.1 for noisy data
# (noise raises background so 0.166 kills targets too)
# ==========================================
img_thresh = img_norm.copy()
img_thresh[img_thresh < 0.1] = 0.0
...
# Background ROIs — offset from each dot
backgrounds_auto = []
...
for k, (ax, dot, bg) in enumerate(zip(axes, dots_auto, backgrounds_auto)):
    ...
    # SBR — signal from img_norm, noise from img_norm (pre-threshold)
...
plt.suptitle("DMAS — Noisy Data (-10dB) | Threshold: 10% of peak\n"
             "Cyan = Signal ROI   Green = Background ROI",
             fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('DMAS_noisy_result.png', dpi=150, bbox_inches='tight')
plt.show()

print(f"\nSBRs: {np.round(all_sbrs, 1)} dB")
print(f"Mean SBR: {np.mean(all_sbrs):.1f} dB")